...
let options = document.getElementById("toggle-options");

let bar = document.querySelector(".bar").offsetHeight;
document.querySelector("main").style.paddingTop = `${bar + 30}px`;

function toggler() {
  console.log("global");
  options.checked = false;
  document.querySelector("html").removeEventListener("click", toggler);
}
...
function setTodayDate(book_slot_no) {
  ...
  let dtStr = `${today.getFullYear() + ""}-${(
    today.getMonth() +
    1 +
    ""
  ).padStart(2, "0")}-${(today.getDate() + "").padStart(2, "0")}`;

  let dtStr2 = `${t.getFullYear() + ""}-${(t.getMonth() + 1 + "").padStart(
    2,
    "0"
...
function bldcfm() {
  document.querySelector(".name-data").textContent =
    document.querySelector(".name-box input").value;
  document.querySelector(".email-data").textContent =
    document.querySelector(".mail-box input").value;
  bcounthid.value = bc;
  for (let i = 1; i <= bc; i++) {
    let dt = books[i].querySelector(".date-box input").value;
    let d = new Date(dt);
    dt = `${(d.getDate() + "").padStart(2, "0")}-${(
      d.getMonth() +
      1 +
      ""
    ).padStart(2, "0")}-${(d.getFullYear() + "").padStart(2, "0")}`;
    let t = d;
    t.setDate(t.getDate() + 15);
    let newdate = `${(t.getDate() + "").padStart(2, "0")}-${(
      t.getMonth() +
      1 +
      ""
...
document.querySelector(".roll-no input").addEventListener("change", (e) => {
